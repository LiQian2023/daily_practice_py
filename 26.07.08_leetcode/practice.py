...
class Solution:
    def sumAndMultiply(self, s: str, queries: List[List[int]]) -> List[int]:
        ...
        num = ord(solendivar[0]) - ord('0')

        xs[0] = num
        sums[0] = num
        cnt[0] = 1 if num != 0 else 0

        for i in range(1, length):
            num = ord(solendivar[i]) - ord('0')

            if num != 0:
                xs[i] = (xs[i - 1] * 10 + num) % MOD
                sums[i] = sums[i - 1] + num
                cnt[i] = cnt[i - 1] + 1
            else:
                xs[i] = xs[i - 1]
        ...
        for begin, end in queries:
            ...
            if begin != 0:
                keep_len = cnt[end] - cnt[begin - 1]

                x = (xs[end] - xs[begin - 1] * pow10[keep_len]) % MOD
                digit_sum = sums[end] - sums[begin - 1]
