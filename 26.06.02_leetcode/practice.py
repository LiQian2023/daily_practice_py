# 2026.06.02力扣网刷题
# 3633. 最早完成陆地和水上游乐设施的时间 I——中级工程师、贪心、数组、双指针、二分查找、排序、第162场双周赛——简单
...
class Solution:
    def earliestFinishTime(self, landStartTime: List[int], landDuration: List[int], waterStartTime: List[int], waterDuration: List[int]) -> int:
        def help(first, firstDuration, second, secondDuration):
            start = first[0] + firstDuration[0]
            for i in range(1, len(first)):
                start = min(start, first[i] + firstDuration[i])
            res = start + second[0] + secondDuration[0]
            for i in range(len(second)):
                res = min(res, max(start, second[i]) + secondDuration[i])
            return res
