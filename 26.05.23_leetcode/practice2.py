...
# 接下来 n 行，依次表示每一扇防御门。每行包括一个字符串 op
# 和一个非负整数 t ，两者由一个空格隔开，且 op 在前，t 在后，op 表示该防御门所对应的操作，t 表示对应的参数。
# 输出格式
...
# 6 XOR 7 = 1
# 类似的，我们可以计算出
# 初始攻击力为 1, 3, 5, 7, 9 时最终攻击力为 0，
# 初始攻击力为 0, 2, 4, 6, 8, 10 时最终攻击力为 1，
# 因此 atm 的一次攻击最多使 drd 受到的伤害值为 1。
# 运算解释
...
# 例如，我们将十进制数 5与十进制数 3分别进行 OR、XOR与 AND运算，可以得到如下结果：
...
class Solution:
    def calc(self, bit: int, nor: int, n: int, datas: List[str])->int:
        for i in range(n):
            data = datas[i].split(' ')
            x = (int(data[1]) >> bit) & 1
            op = data[0]
    ...
    def main(self):
        ...
        list1 = input.pop(0).split(' ')
        n, m = int(list1[0]), int(list1[1])
        ans, val = 0, 0
        for i in range(29, -1, -1):
            bit0 = self.calc(i, 0, n, input)
            bit1 = self.calc(i, 1, n, input)
            if bit0 < bit1 and val + (1 << i) <= m:
                ans += bit1 << i
                val += (1 << i)
            else:
                ans += bit0 << i
