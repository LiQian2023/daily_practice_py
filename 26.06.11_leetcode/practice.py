...
class Solution:
    def perfectMenu(self, materials: List[int], cookbooks: List[List[int]], attribute: List[List[int]], limit: int) -> int:
        cook = [0] * 5
        delicious, full = 0, 0
        ans, n = -1, len(cookbooks)
        def DFS(cook, delicious, full, ans, i):
            for j in range(5):
                if cook[j] > materials[j]:
            ...
            if i == n:
                ...
                return ans, delicious, full
            for j in range(5):
                cook[j] += cookbooks[i][j]
            delicious += attribute[i][0]
            full += attribute[i][1]
            ans, delicious, full = DFS(cook, delicious, full, ans, i + 1)
            for j in range(5):
                cook[j] -= cookbooks[i][j]
            delicious -= attribute[i][0]
            full -= attribute[i][1]
            ans, delicious, full = DFS(cook, delicious, full, ans, i + 1)
            return ans, delicious, full
        ans, delicious, full = DFS(cook, delicious, full, ans, 0)
        return ans