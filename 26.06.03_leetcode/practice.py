# 2026.06.03力扣网刷题
# 3635. 最早完成陆地和水上游乐设施的时间 II——资深工程师、贪心、数组、双指针、二分查找、排序、第162场双周赛——中等
...
class Solution:
    def earliestFinishTime(
        ...
        def help(first, firstDuration, second, secondDuration):
            begin = first[0] + firstDuration[0]
            for i in range(1, len(first)):
                begin = min(begin, first[i] + firstDuration[i])
            res = begin + second[0] + secondDuration[0]
            for i in range(len(second)):
                res = min(res, max(begin, second[i]) + secondDuration[i])
            return res
        land = help(landStartTime, landDuration, waterStartTime, waterDuration)
        water = help(waterStartTime, waterDuration, landStartTime, landDuration)
