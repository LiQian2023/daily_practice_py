...
class VerNode:
    def __init__(self, ver):
        self.first = EdgeNode(ver)
class ALGraph:
    def __init__(self, n):
        self.verNum = n
        self.verList = []
        for i in range(n):
            self.verList.append(VerNode(-1))
    
    def Push(self, begin, end):
        p = EdgeNode(end)
        p.next = self.verList[begin].first.next
        self.verList[begin].first.next = p
    
    def DFS(self, k, visited):
        if visited[k]:
            return
        visited[k] = True
        if self.verList[k].first.next == None:
            return
        p = self.verList[k].first.next
        while p:
            if not visited[p.adjVer]:
                self.DFS(p.adjVer, visited)
            p = p.next
...
class Solution:
    def remainingMethods(self, n: int, k: int, invocations: List[List[int]]) -> List[int]:
        ...
        for invocation in invocations:
            obj.Push(invocation[0], invocation[1])
        # 访问标记
        visited = [False] * n
        obj.DFS(k, visited)
        Del = True
        for invocation in invocations:
            begin, end = invocation[0], invocation[1]
            if not visited[begin] and visited[end]:
                Del = False
                break
        ans = []
        if not Del:
